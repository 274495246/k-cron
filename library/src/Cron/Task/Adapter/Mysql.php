  ...
  PRIMARY KEY (`id`)
    ...
    private function init()
    {
        ...
        if (!empty($data) && intval($data["total"]) == 0) {
            ...
            if ($stmt) {
                Log::log_write("执行sql:" . $this->createTable . "执行成功");
            } else {
                Log::log_write("执行sql:" . $this->createTable . "执行失败");
            }
        }
    }
    ...
    {
        $config = Loader::import(CRON_CONF_PATH . "/config.php");
        if (empty($config) || !isset($config["mysql"])) {
            Log::log_write("mysql config not found");
            exit();
        }
        return $config["mysql"];
    }
